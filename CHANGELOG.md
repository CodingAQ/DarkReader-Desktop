...
### 修复
 ...
 - **作用域状态恢复重构**：引入 `EffectScope`（0=全屏, 1=区域, 2=窗口）作为作用域的唯一权威来源，替代原来依赖 `UseRegion`/`UseWindow` 的判断。此前若目标窗口在关闭期间被销毁，启动后会回退到全屏模式；修复后窗口模式始终恢复，等待窗口重开。旧配置自动迁移
...
### 变更
 ...
 - 托盘菜单重构：合并"Select Region"/"Clear Region"/"Select Window"/"Clear Window Target"为统一的作用域菜单（Full Screen / Select Region / Select Windows），通过 `SetScope` 统一处理作用域切换
